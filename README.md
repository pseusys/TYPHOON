...
While the structure is random by nature, it's strictly described in the certificate for every server flow manager (unique IP address - port number tuple), so that all the traffic going to and from this address would have it uniform.
