...
- Health check provider (one per session): attached to session controller, keeps internal protocol state, manages handshake message timers and injects shake messages themselves if necessary.
...
- Decoy provider (one pre flow): attached to flow controller, observes flow packet stream and injects decoy packets whenever necessary.
