...
> Normally only one of these values should be set, but there is an exception: a health check packet (that normally has an empty body) can be embedded into a data packet, that situation here and after is called "shadowride".
...
There is no reason to wait for the third packet form client (TCP-style) as it is not possible to overload server with partially-initialized sessions, because [only one active connection is allowed per certificate](#sockets-and-listeners).
...
- For `ESK` and `EPK` [Classic McEliece](TODO) algorithm is used.
- For `VSK` and `VPK` [Ed25519](TODO) algorithm is used (even though it's not post-quantum, it's not too bad, since even if it's broken the handshake will not succeed after all).
- `OBFS` consists of just random bytes.

For `full` mode:

- For `ESK` and `EPK` [X25519](TODO) algorithm is used.
- For `VSK` and `VPK` [Ed25519](TODO) algorithm is used.
- For `OSK` and `OPK` [X25519](TODO) and [Elligator2](TODO) algorithms are used.
...
There are only two packets that have to be encrypted and decrypted: cliemt handshake and server handshake.
...
3. Client builds a transcript by applying `SHA256` hashing on `CliShrSec`, `SrvShrSec`, `CliNnc` and `SrvNnc`, producting `Trans`.
...
After the server initiates the internal state for the usert and waits for an appropriate time, it encrypts the client response using the following steps:

0. Client comes up with initial data (might be just a random byte string).
...
4. Client obfuscates `SrvEphPubKey` using a mode-dependant method (see below), producing `SrvEphPubKeyObf`.
...
8. Client encrypts the handshake tailor with [tailor encryption](#tailor-encryption) algorithm.
9. Client encrypts initial data with [marshalling encryption](#marshalling-encryption) algorithm with `Sess` as a key.
10. Client constructs the handshake encrypted tailor by concatenating `SrvEphPubKeyObf`, `TransAuth`, `SrvNnc` and the encrypted tailor itself.
