...
OBFS4_ARGS=$(<  /keys/obfs4_args.txt)
...
obfs4proxy -enableLogging -logLevel INFO 2>/dev/null | while IFS= read -r line; do
    ...
    case "${line}" in
        CMETHOD\ obfs4\ socks5\ *)
            ...
            ADDR=$(echo "${line}" | awk '{print $4}') || true
            SOCKS5_PORT=$(echo "${ADDR}" | cut -d: -f2) || true
...
for i in {1..30}; do
    if [[ -f /tmp/socks5_port ]]; then break; fi
    sleep 1
done
SOCKS5_PORT=$(<  /tmp/socks5_port)
