...
*How does the TYPHOON server hold up under load?*
The **dockerized flood** is the core: the eval TYPHOON client/server containers (`EVAL_MODE=load`) run the realistic obfuscated profile over a real veth network (`compose/docker-compose.load.yml`), where `docker stats` samples **both** containers' memory (peak + growth slope, to test server queue-memory growth) and CPU, and the logs give throughput + packet loss (application sequence gaps plus the core `drain_drops` / `recv_errors` counters)
 Two optional host add-ons share one Rust impl (`benches/load.rs`): the criterion `load` bench (`--criterion`, in-process one-way send throughput — the raw send-path ceiling per flow count) and a `perf` flamegraph of that bench (`--flamegraph`, the in-process send/recv hot path).
Linux + a running **Docker** daemon; the criterion/flamegraph add-ons also need `cargo` (and `perf` + `cargo-flamegraph` for the flamegraph) and auto-skip when those are missing.
Mirrors `.github/workflows/evaluation.yaml`.
...
- Optional, Linux only: `perf` + [`cargo-flamegraph`](https://github.com/flamegraph-rs/flamegraph) — enables Part 4 (`load-test`). Without them, `poe load-test` / the pipeline's `loadtest` phase is skipped.
  Flamegraphs additionally need perf sampling access: set `kernel.perf_event_paranoid` low enough (see [Part 4](#part-4--load--stress-testing-cli)) — otherwise `cargo flamegraph` fails and the flamegraph step is skipped (the criterion throughput + process CPU/RSS/loss numbers are still produced).
...
### Part 4 — Load / stress testing (CLI)

```shell
poe load-test                                                  # default grid: 5,10,20 MB × 1,2,4 flows × 1,2 readers
poe load-test --sizes 5 --flows 1,2 --readers 1,2 --profile bulk_upload   # smaller sweep
poe load-test --no-build                                       # reuse already-built docker images
poe load-test --no-criterion --no-flamegraph                   # docker flood only (fastest)
```

Linux + a running **Docker** daemon (the flood runs the eval TYPHOON client/server containers via `compose/docker-compose.load.yml`). The `--criterion` and `--flamegraph` host add-ons are on by default and auto-skip when their deps (`cargo`; `perf` + `cargo-flamegraph` for the flamegraph — see `.github/workflows/evaluation.yaml` for the one-time perf setup) are missing. It produces `load_test.pdf` (host send throughput / docker flood throughput / per-container peak memory + growth / loss panels), `load_test_table.md` (per-cell table + auto-derived findings incl. the server memory-growth verdict), `load_test.json`, and — when the flamegraph runs — `flamegraphs/load.svg` (interactive) plus a `.pdf` via `rsvg-convert` (`librsvg2-bin`).
...
Alternatively, run the load test under `sudo` (perf as root ignores the paranoid setting), though that also builds/runs cargo as root.

### Pipeline

```shell
poe evaluate                    # build → capture → analyze → Part 1 + 2 plots → background → loadtest → report.md
...
poe evaluate --skip loadtest    # skip the load-test sweep + flamegraph (auto-skipped on non-Linux anyway)
...
poe evaluate --classification-runs 1 --typhoon-runs 2 --typhoon-uc-runs 1 \
    ...
    --loadtest-sizes 5 --loadtest-flows 1,2 --loadtest-readers 1,2 --loadtest-repeats 1 --quiet-build
