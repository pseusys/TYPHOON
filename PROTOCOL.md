...
The header is appended _after_ the payload, so hereinafter it will be called the "trailer" instead (just like in IPSec, for instance).
Its structure will be explained [below](#trailer-structure), and it is encrypted with [trailer encryption](#trailer-encryption).
...
The trailer should always be positioned _at the very end_ of a TYPHOON packet.
The trailer structure consists of the following fields (total: `16 + TYPHOON_ID_LENGTH` bytes):
...
The trailer is the only part of the message that should be decrypted by the flow manager.
It always starts at `packet_length - trailer_length - trailer_encryption_overhead` and ends at the end of the packet.
If the data flag is set, the payload should be read starting from `packet_length - trailer_length - trailer_encryption_overhead - payload_length` and until the start of the trailer.
If the decoy flag is set, the packet should be discarded right away by the flow manager.
If trailer decryption or authentication fails, the raw (still-encrypted) packet is forwarded to the flow manager's [active probe handler](#active-probing-protection) instead of being silently dropped.
...
The server keeps a set of clients mapped to their unique identifiers (**ID** trailer field), but it is never notified about client proxy selection.
...
sequenceDiagram
    ...
    SM->>SM: Build trailer (flags=DATA, PN, ID)
    SM->>FM: deliver(body ∥ trailer)
    FM->>FM: Prepend FakeHeader + FakeBody
    FM-->>FM: UDP datagram sent

    FM-->>FM: receive UDP datagram
    FM->>FM: Strip FakeHeader + FakeBody
    alt trailer decryption / authentication ok
        FM->>FM: Decrypt trailer → verify identity + PN
        FM-->>SM: deliver(body)
        SM->>SM: Decrypt body (session key)
        SM-->>App: recv() → plaintext payload
    else trailer decryption / authentication fail
...
Just like it has already been described in [packet structure](#packet-structure) section, every data packet is encrypted, prefixed with a [fake header](#fake-header) and postfixed with an [encrypted trailer](#trailer-structure).
...
The handshake packets carry implementation-dependent encrypted initial data and also are prefixed with a [fake header](#fake-header) and postfixed with an [encrypted trailer](#trailer-structure).
...
The decay behavior is mostly defined by [**TM** and **PN** fields of the trailer](#trailer-structure).
...
A fraction of decoy packets — controlled per-flow — may be emitted as **fallthrough** decoys: the wire packet is built without the [trailer](#trailer-structure) section.  Such a packet carries no flags, no payload-length field, no packet number, and no identity; it is just `[fake_header? | fake_body | random_body]`, where each piece is generated by the normal flow-manager pipeline (the fake header / fake body may be empty exactly as for a regular packet whose dice did not roll for them).

Fallthrough decoys broaden TYPHOON's wire-size distribution into the band that the trailer + identity + AEAD-tag overhead otherwise puts a hard floor on, letting the protocol blend with the long tail of arbitrary UDP traffic (NAT mishaps, ICMP-shaped echoes, malformed gaming heartbeats, niche control-plane protocols).

Each flow draws a single probability at initialization from `[TYPHOON_DECOY_FALLTHROUGH_PACKETS_MIN, TYPHOON_DECOY_FALLTHROUGH_PACKETS_MAX]` (asserted floats in `[0, 1]`, default range `[0.0, 0.25]`) and holds it constant; every subsequent decoy emission rolls a coin against that probability to decide whether to emit a fallthrough decoy or a regular trailered one.  A custom decoy factory may override the per-flow range to bias a specific provider toward more (or fewer) fallthrough decoys without changing global settings.

> NB! Valid fallthrough decoys arriving at the peer fail trailer authentication and therefore land in the [active probe handler](#active-probing-protection) bucket alongside genuine probes.
> The handler must tolerate the rate `MAX × baseline_decoy_rate` of failed trailer decryptions per peer without escalating heuristics.
...
To resist this, the flow manager must handle packets that fail trailer authentication gracefully rather than closing the socket or producing a distinctive error response.
...
- **Undersized wire packet (no trailer header)**: any datagram shorter than `identity_len + trailer_overhead` cannot carry a valid encrypted trailer and is forwarded as a probable probe.
- **Trailer decryption failure**: the trailer cannot be deciphered with the flow's receive key.
- **Trailer verification failure**: the trailer decrypts but its BLAKE3/AEAD authentication tag does not verify against the identity's per-user obfuscation key. On the server this also catches non-handshake packets whose identity is absent from the global registered-users map — `verify_trailer` performs the global lookup as part of the same step.
...
6. Client encrypts the handshake trailer with [trailer encryption](#trailer-encryption) algorithm (NB! Here initial data encryption key is used for additional data instead of session key).
7. Client constructs the handshake packet by concatenating `CliEphPubKeyObf`, `CiphObf`, `CliNnc` and encrypted initial data as the body. The handshake trailer is appended to the body and encrypted by the flow manager, as with all other packets.
...
The [handshake trailer](#handshake-packets) is appended after this body and encrypted separately by the flow manager, exactly as with every other packet.
...
6. Client extracts the server-generated identity from the server handshake trailer, adopting it for all subsequent communication.
...
8. Server encrypts the handshake trailer with [trailer encryption](#trailer-encryption) algorithm (NB! Here initial data encryption key is used for additional data instead of session key, same as in the client handshake step 6). Server upgrades to the session key after sending the response.
...
10. Server constructs the handshake response by concatenating `SrvEphPubKeyObf`, `TransAuth`, `SrvNnc` and encrypted initial data as the body. The handshake trailer is appended to the body and encrypted by the flow manager, as with all other packets.
11. The payload is sent to the client inside of a handshake packet. The server trailer contains the server-generated identity for the client to use in subsequent communication.
...
Trailer encryption differs significantly depending on cryptographic mode chosen.
Since the trailer is appended to every single TYPHOON packet, the encryption performance requirements here are high.

The requirements for trailer encryption on client and server are fundamentally different.
Since the server expects packets from multiple clients, it has to decrypt all the trailers with a single algorithm.
At the same time, the client expects packets from one server only and can use private session secrets for decryption.

#### Trailer encryption in `fast` mode

In case of the `fast` mode (which should be used if large amounts of data are expected to be transferred), trailer is encrypted using [marshalling encryption](#marshalling-encryption) algorithm with `OBFS` used for key.
Additionally, after encryption, the ciphertext is also authenticated with `BLAKE3` using the session key; that helps to make sure that trailer was not modified, even if it was decrypted.
...
```mermaid
flowchart LR
    subgraph "Fast-mode encrypted trailer — both directions (left = start)"
        CN["Ciphertext ∥ Nonce: trailer encrypted with OBFS (56 B software / 48 B hardware)"]
...
#### Trailer encryption in `full` mode

In case of the `full` mode (which should be used if obfuscation should be able to resist certificate leaking), trailer encryption differs depending on the packet flow direction.

For the packets going from server to client, trailers are simply encrypted with [marshalling encryption](#marshalling-encryption) algorithm, that provides maximal privacy and efficiency.
That does not allow anyone without client session key to decrypt the trailer contents, which is fine in case no one else is intended to read it anyway.

```mermaid
flowchart LR
    subgraph "Full-mode encrypted trailer — server → client (left = start)"
        CNT["Ciphertext ∥ Nonce ∥ Tag: trailer encrypted with session key (72 B software / 60 B hardware)"]
    end
```

For the packets going from client to server, trailers are encrypted using the following steps:

0. Client comes up with the raw trailer `Trailer`.
...
5. Client encrypts the `Trailer` using [marshalling encryption](#marshalling-encryption) using `BLAKE3` on `ShrSec` as key and `Nnc` as additional data, producing `TrailerEnc`.
6. Client constructs the encrypted tail by concatenating `TrailerEnc`, `EphPubKeyObf` and `Nnc`.

```mermaid
flowchart LR
    subgraph "Full-mode encrypted tail — client → server (left = start)"
        TE["TrailerEnc: trailer encrypted with BLAKE3(ShrSec) — ciphertext ∥ nonce ∥ tag (72 B software / 60 B hardware)"]
...
The trailers are decrypted using the following steps:

0. Server extracts `TrailerEnc`, `EphPubKeyObf` and `Nnc` from the client message.
...
3. Server decrypts the `TrailerEnc` using [marshalling encryption](#marshalling-encryption) using `BLAKE3` on `ShrSec` as key and `Nnc` as additional data, producing `Trailer`.

That approach allows safe encryption guarantees for all trailers.
For the packets going from client to server, the transmitted trailer structure can still be deobfuscated if the certificate leaks; this unfortunately cannot be avoided.
Still, even if it is deobfuscated, the trailer contents remain encrypted and will not leak.
...
| `OBFS` _(fast mode)_ | 32-byte symmetric key | Pre-shared key used for [trailer obfuscation](#trailer-encryption) |
...
| `OSK` _(full mode)_ | [X25519](#marshalling-encryption) static secret | Decrypts client-to-server trailers via ephemeral X25519 exchange |
...
| `OBFS` _(fast mode)_ | 32-byte symmetric key | Pre-shared [trailer obfuscation](#trailer-encryption) key (same value as in the server key pair) |
| `OPK` _(full mode)_ | [X25519](#marshalling-encryption) public key | Server's long-term public key for encrypting client-to-server trailers |
...
The session manager keeps track of session health, encryption, and data transfer, while flow managers send decoy packets, obfuscate traffic, and can only decrypt packet trailers.
...
- Active probe handler (one per flow): attached to flow controller, receives every packet that could not be identified — undersized wire packet, trailer decryption failure, or trailer verification failure (which on the server also catches non-handshake packets from globally-unregistered identities) — and may send a raw response via the flow socket to mimic another protocol.

#### Identification and rebinding

As is evident from the [trailer structure](#trailer-structure), the trailer carries a constant number of bytes for client authentication.
These bytes are used for packet demultiplexing: all TYPHOON packets arrive at the same UDP socket of a flow manager but are delivered to different logical session managers based on the **ID** trailer field.
...
- Session key authentication that is used for [trailer encryption](#trailer-encryption).
- Incremental packet number [in trailer](#trailer-structure), stored in the higher `4` bytes of **PN** field so it alone determines packet ordering. A single monotonic counter is maintained per session and shared by every emitter — data, health-check, handshake, decoy, and termination packets all advance the same sequence. Keeping the counter (rather than the timestamp) in the dominant half means PN ordering for source-address rebinding cannot be disturbed by clock adjustments — only the counter, which never goes backward, decides it.
...
A scenario when an attacker impersonates a user, forging a fake packet on their behalf, should not be a concern, since [trailer structure is authenticated with user session key anyway](#trailer-encryption).
...
The only requirement for this is packet validation, which is [performed using the user session key](#trailer-encryption) — this key is pulled from the global user table.
...
The client embeds its application version into the **ID** field of the handshake trailer.
...
- The **ID** trailer field length (as [mentioned earlier](#identification-and-rebinding)) should be chosen based on the implementation's identification requirements.
...
| Trailer encryption (nonce + auth tags) | 72 B | sum of `NONCE_LEN` + `SYMMETRIC_BUILT_IN_AUTH_LEN` + `SYMMETRIC_ADDITIONAL_AUTH_LEN` |
...
| **Floor for decoy / health-check packets** | **88 B** | identity + trailer only; no payload AEAD |
...
It reinterprets three [trailer fields](#trailer-structure) to carry diagnostic metadata instead of their production meanings:
...
Flow managers use `before_cap` to prepend fake headers and fake bodies without copying the payload, and `after_cap` to append the encrypted trailer.
...
- `Symmetric` — wraps the feature-selected AEAD cipher (XChaCha20-Poly1305 or AES-GCM-256). `encrypt_auth` / `decrypt_auth` for authenticated payload encryption; `decrypt_no_verify` + `verify_decrypted` for the two-step fast-mode trailer path.
- `encrypt_anonymously` / `decrypt_anonymously` — unauthenticated stream cipher (XChaCha20 or AES-256-CTR) used to obfuscate public keys in the handshake and in full-mode trailer encryption.
...
- `ClientCryptoTool<T>` — client-side per-flow handle. Holds the session cipher via a `CachedValue` so the key upgrade after handshake is visible to all flows with no locking. Obfuscates client-to-server trailers and decrypts server-to-client trailers.
- `ServerCryptoTool<T>` — server-side per-flow handle. Holds two `CachedMapEntryTemplate` snapshots (send and receive directions) over the shared user table. Deobfuscates client-to-server trailers, verifies incremental packet numbers, and encrypts server-to-client trailers.
...
The `trailer` module provides the `Trailer<T>` struct — a strongly-typed view over the fixed-size plaintext bytes at the end of every wire packet — and the `IdentityType`, `ServerConnectionHandler`, `ClientConnectionHandler` extension traits.

`Trailer` constructors (`data`, `health_check`, `handshake`, `decoy`, `termination`, `debug_probe`) write the appropriate flag byte, code, time, packet number, payload length, and identity into a pre-allocated `DynamicByteBuffer` in one pass with no copies. There is no dedicated `shadowride` constructor — a shadowridden packet is built by mutating an existing `data` trailer's flags/time/PN in place once a pending health check is ready to attach (see `feed_output` on both the client- and server-side health providers).
...
Its send path prepends fake header + fake body and appends the encrypted trailer before writing to the socket.
...
Both managers delegate trailer encryption/decryption to a `FlowCryptoProvider` implementation (`ClientCryptoTool` or `ServerCryptoTool`), keeping the flow layer agnostic to the crypto mode.
...
| 267654 | 32 | OBFS | Symmetric trailer obfuscation key |
| **267686** | — | EOF | |
...
| 261162 | 32 | OBFS | Symmetric trailer obfuscation key |
...
The packet payload never gets decrypted—only the trailer is parsed.
...
**The challenge**:
Configuration of this type of proxy would require changing how trailer encryption works, allowing packet data encryption and trailer obfuscation to use different keys (i.e., packet data is encrypted using the original end-to-end session key, while the packet trailer is authenticated using the session key of the last-hop proxy).
In addition to that, extended delays of the packets going through the complex path would require a way of notifying the client that their original packet is not lost, but still haven't reached its destination: a special "wait more" trailer flag is suggested for adding, that would reset the client decay cycle, but not advance any counters or change any values.
...
Because fake headers and fake bodies are purely decorative and do not affect how the receiving side locates or parses the trailer (the trailer position is always derived from known length constants, not from the fake header contents), neither side needs to know or agree on the other side's current configuration.
