...
use typhoon::socket::{ServerBuilder, ServerFlowConfiguration};
...
async fn test_multi_client_isolated_sessions() {
    ...
    let listener = Arc::new(ServerBuilder::<StaticByteBuffer, DefaultExecutor, DefaultServerConnectionHandler>::new(key_pair, DefaultServerConnectionHandler).add_flow(ServerFlowConfiguration::with_address(empty_flow_config(), addr)).with_settings(settings.clone()).build_listener().await.expect("listener"));
