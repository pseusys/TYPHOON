...
///      sanctioned way to defer real traffic.  The plaintext trailer is
...
struct FlatIatState {
    /// FIFO of (PN, body || plaintext-trailer) entries held back from immediate
    /// send.  Re-injection preserves the original trailer (including the DATA
    ...
    /// mark it in `reinjected` without re-parsing the trailer.
        ...
        loop {
            ...
            let (packet, fallthrough) = {
                ...
                if let Some((pn, real_packet)) = guard.queue.pop_front() {
                    ...
                    let total = body_len + Trailer::<T>::len();
                    let buf = settings.pool().allocate(Some(total));
                    rand::thread_rng().fill(buf.slice_end_mut(body_len));
                    Trailer::decoy(buf.rebuffer_start(body_len), &identity.get(), guard.next_packet_number());
...
impl<T: IdentityType + Clone + 'static, AE: AsyncExecutor + 'static> DecoyProvider for FlatIatDecoyProvider<T, AE> {
    ...
    async fn feed_output(&self, body: DynamicByteBuffer, trailer_buf: DynamicByteBuffer) -> Option<DynamicByteBuffer> {
        ...
        // discards the plaintext trailer the client needs to complete the handshake.
        if !flags.has_payload() {
            return Some(body);
        }
        let pn_bytes: [u8; 8] = trailer_buf.slice()[PN_OFFSET..PN_OFFSET + 8].try_into().expect("8-byte PN");
        ...
        // Fresh app-level send: stash (body || plaintext-trailer) for the timer.
        // `body` and `trailer_buf` are two adjacent views into the same backing
        // allocation (split by `prepare_outgoing`).  Stretching body's end by
        // `trailer_buf.len()` re-covers the trailer in place — no copy, no
        // self-overlap UB that `append_buf` would trigger.
        let combined = body.expand_end(trailer_buf.len());
