...
use crate::trailer::{ClientConnectionHandler, IdentityType, Trailer};
...
impl<T: IdentityType + Clone + 'static, AE: AsyncExecutor + 'static, CC: ClientConnectionHandler + 'static> ClientSocketBuilder<T, AE, CC> {
    ...
    pub async fn build(mut self) -> Result<ClientSocket<T, AE, CC>, ClientSocketError> {
        ...
        let trailer_wire_len = Trailer::<T>::encrypted_len_c2s();
        ...
        for (addr, config) in addr_configs {
            ...
            max_data_payload = max_data_payload.min(config.max_user_payload(settings.mtu(), PAYLOAD_CRYPTO_OVERHEAD, trailer_wire_len));
