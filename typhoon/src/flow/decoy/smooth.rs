...
use crate::trailer::{IdentityType, PacketFlags};
...
pub struct SmoothDecoyProvider<T: IdentityType + Clone + 'static, AE: AsyncExecutor + 'static> {
    manager: Weak<dyn DecoyFlowSender>,
    state: Arc<RwLock<DecoyState<T, AE>>>,
}

impl<T: IdentityType + Clone, AE: AsyncExecutor> SmoothDecoyProvider<T, AE> {
    ...
    async fn timer_task(manager: Weak<dyn DecoyFlowSender>, state: Arc<RwLock<DecoyState<T, AE>>>) {
        loop {
            ...
            let Some(manager_arc) = manager.upgrade() else {
                warn!("SmoothDecoyProvider: manager dropped, stopping timer");
                break;
            };
...
impl<T: IdentityType + Clone + 'static, AE: AsyncExecutor + 'static> DecoyProvider for SmoothDecoyProvider<T, AE> {
    ...
    async fn feed_output(&self, body: DynamicByteBuffer, trailer_buf: DynamicByteBuffer) -> Option<DynamicByteBuffer> {
        ...
        if !flags.is_discardable() {
            let mut state = self.state.write().await;
            state.update(body.len() + trailer_buf.len(), true);
        }
        Some(body)
    }
}

impl<T: IdentityType + Clone, AE: AsyncExecutor> DecoyCommunicationMode<T, AE> for SmoothDecoyProvider<T, AE> {
