...
pub trait DecoyFlowSender: Send + Sync {
    /// Send a generated decoy packet through the flow manager. `fallthrough` skips the trailer step (see `FlowManager::send_packet`).
...
pub trait DecoyProvider: Send + Sync {
    ...
    /// `trailer_buf` is the deobfuscated trailer for the packet (flags, packet number, etc.).
    async fn feed_input(&self, packet: DynamicByteBuffer, trailer_buf: DynamicByteBuffer) -> Option<DynamicByteBuffer>;

    /// Process an outgoing packet body and its plaintext trailer, updating internal rate tracking.
...
pub(crate) struct DecoyState<T: IdentityType + Clone, AE: AsyncExecutor> {
    ...
    /// Live source of the current session identity for decoy trailers; re-read on every emitted
...
impl<T: IdentityType + Clone, AE: AsyncExecutor> DecoyState<T, AE> {
    ...
    /// Roll a coin against `fallthrough_probability`; `true` ⇒ next decoy bypasses the trailer.
    #[inline]
    ...
    pub(super) fn create_decoy_packet(&mut self, body_length: usize, is_maintenance: bool) -> DynamicByteBuffer {
        ...
        let total_length = body_length + Trailer::<T>::len();
        ...
        Trailer::decoy(packet.rebuffer_start(body_length), &self.identity.get(), pn);
    ...
    /// Create a replica of the given decoy body (same body bytes, new trailer).
