...
cfg_if! {
    if #[cfg(feature = "client")] {
        ...
        use crate::crypto::{ClientCryptoTool, TRAILER_C2S_OVERHEAD};
...
pub(crate) trait FlowManager {
    ...
    /// * `fallthrough` is set only by fallthrough decoys and skips the trailer step in `prepare_outgoing`; all other callers pass `false`.
...
/// Outcome of processing an incoming packet through trailer decryption and verification.
#[cfg(feature = "client")]
pub(crate) enum ProcessIncomingResult {
    /// Trailer verified: packet returned for session-layer processing.
...
impl<T: IdentityType + Clone> FlowSendInternal<T> {
    /// Encrypt trailer, add fake header and body, return assembled packet ready for socket send.
    /// * When `fallthrough` is set, the trailing plaintext trailer bytes are dropped and the trailer-encryption step is skipped --- only fake header / body padding is added on top of the body.
    ...
    pub(crate) fn prepare_outgoing(&mut self, packet: DynamicByteBuffer, mtu: usize, pool: &BytePool, fallthrough: bool, is_maintenance: bool) -> Result<DynamicByteBuffer, FlowControllerError> {
        let full_trailer_len = Trailer::<T>::len();

        let (encrypted_packet, packet_flags, data_len) = if fallthrough {
            // Fallthrough decoy: the input is `[random_body | plaintext_trailer]` (the trailer was written for accounting only) - truncate it to the body and forward as opaque noise.
            let body_only = packet.rebuffer_end(packet.len() - full_trailer_len);
            let body_len = body_only.len();
            (body_only, PacketFlags::DECOY, body_len)
        } else {
            let (packet_data, packet_trailer) = packet.split_buf_end(full_trailer_len);
            let flags = PacketFlags::from_bits_truncate(*packet_trailer.get(0));
            let data_len = packet_data.len();
            let encrypted = match self.provider.get_mut().map_err(FlowControllerError::MissingCache)?.obfuscate_trailer(packet_trailer, pool) {
                Ok(res) => packet_data.expand_end(res.len()),
                Err(err) => return Err(FlowControllerError::TrailerEncryption(err)),
            };
        ...
        self.capture.record_send(|| {
            ...
            let trailer_overhead = if fallthrough {
                0
            } else {
                TRAILER_C2S_OVERHEAD
            };
            ...
            (kind, trailer_len, trailer_overhead, fake_header_len, data_len, full_packet_len - fake_header_len)
        });
...
impl<T: IdentityType + Clone> FlowReceiveInternal<T> {
    /// Deobfuscate the trailer from a raw wire packet.
    /// Returns `Ok(Some((body, trailer_buf)))` on success, `Ok(None)` on crypto failure
    ...
    pub(crate) fn deobfuscate_incoming(&mut self, packet: DynamicByteBuffer, pool: &BytePool) -> Result<Option<(DynamicByteBuffer, DynamicByteBuffer)>, FlowControllerError> {
        let encrypted_trailer_len = Trailer::<T>::encrypted_len_s2c();
        // A wire packet shorter than the encrypted trailer cannot be a valid Typhoon
        ...
        if packet.len() < encrypted_trailer_len {
            warn!("client flow: undersized wire packet ({} < {encrypted_trailer_len})", packet.len());
            return Ok(None);
        }
        let (body, encrypted_trailer) = packet.split_buf_end(encrypted_trailer_len);
        let cipher = self.provider.get_mut().map_err(FlowControllerError::MissingCache)?;
        match cipher.deobfuscate_trailer(encrypted_trailer, pool) {
            Ok((trailer_buf, transcript)) => match cipher.verify_trailer(transcript) {
                Ok(()) => Ok(Some((body, trailer_buf))),
                Err(err) => {
                    warn!("client flow: trailer verification failed: {err}");
                    Ok(None)
                }
            },
            Err(err) => {
                warn!("client flow: trailer decryption failed: {err}");
                Ok(None)
            }
        }
    }
    ...
    pub(crate) fn process_with_trailer(&self, body: DynamicByteBuffer, trailer_buf: DynamicByteBuffer) -> ProcessIncomingResult {
        let full_trailer_len = Trailer::<T>::len();
        let Some(trailer) = Trailer::<T>::validated(trailer_buf, body.len()) else {
            warn!("client flow: malformed trailer (size, flags or payload_length out of range), dropping packet");
            return ProcessIncomingResult::Decoy;
        };
        if trailer.flags().is_discardable() {
            return ProcessIncomingResult::Decoy;
        }
        let payload_len = trailer.payload_length() as usize;
        ProcessIncomingResult::Valid(body.rebuffer_start(body.len() - payload_len).expand_end(full_trailer_len))
    }
}
