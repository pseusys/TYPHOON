...
use crate::crypto::{ObfuscationTranscript, ServerCryptoTool, TRAILER_S2C_OVERHEAD};
...
use crate::trailer::{IdentityType, PacketFlags, Trailer};
...
impl<T: IdentityType + Clone + Eq + Hash + Send + ToString + 'static, AE: AsyncExecutor + 'static> ServerFlowManager<T, AE> {
    ...
    /// Receive a raw packet, deobfuscating the trailer but returning the full body + trailer view.
    ...
    pub(crate) async fn receive_raw(&self, packet: DynamicByteBuffer, sock: &Socket) -> Result<RawReceivedPacket<T>, FlowControllerError> {
        let encrypted_trailer_len = Trailer::<T>::encrypted_len_c2s();

        loop {
            let (packet, source_addr) = sock.recv_from(packet.clone()).await.map_err(FlowControllerError::Socket)?;

            // Undersized wire packets (shorter than the encrypted trailer) can't be valid Typhoon; forward to the probe handler and keep draining.
            if packet.len() < encrypted_trailer_len {
                warn!("server flow: undersized wire packet from {source_addr} ({} < {})", packet.len(), encrypted_trailer_len);
                self.probe_handler.lock().await.process(packet, Some(source_addr)).await;
                continue;
            }

            let (encrypted_packet, encrypted_trailer) = packet.split_buf_end(encrypted_trailer_len);

            // Deobfuscate trailer; for non-handshake packets verify immediately, for handshake packets defer the HMAC check until the listener has decapsulated the body and can recompute the initial-data encryption key.
            let (trailer, handshake_transcript) = {
                let mut crypto = self.crypto_recv.lock().await;
                let (trailer_buf, transcript) = match crypto.deobfuscate_trailer(encrypted_trailer, self.settings.pool()) {
                    Ok(result) => result,
                    Err(err) => {
                        warn!("server flow: trailer decryption failed from {source_addr}: {err}");
                        self.probe_handler.lock().await.process(encrypted_packet.expand_end(encrypted_trailer_len), Some(source_addr)).await;
                        continue;
                    }
                };
                let Some(trailer) = Trailer::<T>::validated(trailer_buf, encrypted_packet.len()) else {
                    warn!("server flow: malformed trailer from {source_addr} (size, flags or payload_length out of range)");
                    self.probe_handler.lock().await.process(encrypted_packet.expand_end(encrypted_trailer_len), Some(source_addr)).await;
                    continue;
                };
                if trailer.flags().contains(PacketFlags::HANDSHAKE) {
                    (trailer, Some(transcript))
                } else {
                    let identity = trailer.identity();
                    if let Err(err) = crypto.verify_trailer(&identity, transcript).await {
                        debug!("error verifying packet trailer: {err}");
                        self.probe_handler.lock().await.process(encrypted_packet.expand_end(encrypted_trailer_len), Some(source_addr)).await;
                        continue;
                    }
                    (trailer, None)
            ...
            let packet_flags = trailer.flags();
            let identity = trailer.identity();
            ...
            if !packet_flags.contains(PacketFlags::HANDSHAKE) {
                ...
                if let Some(dp) = dp {
                    let notified = dp.feed_input(encrypted_packet.clone(), trailer.buffer().clone()).await;
                    if notified.is_none() {
                        continue;
                    }
                }
            ...
            let original_wire_packet = packet_flags.contains(PacketFlags::HANDSHAKE).then(|| encrypted_packet.expand_end(encrypted_trailer_len));
            ...
            let body = if packet_flags.contains(PacketFlags::HANDSHAKE) {
                let payload_len = trailer.payload_length() as usize;
...
impl<T: IdentityType + Clone + Eq + Hash + Send + ToString + 'static, AE: AsyncExecutor + 'static> ServerFlowManager<T, AE> {
    ...
    pub(crate) async fn send_packet(&self, packet: DynamicByteBuffer, fallthrough: bool, is_maintenance: bool) -> Result<(), FlowControllerError> {
        let trailer_len = Trailer::<T>::len();
        let (body, trailer_buf) = packet.split_buf_end(trailer_len);
        let identity = ServerCryptoTool::<T>::extract_identity(&trailer_buf);

        // Feed decoy provider for rate tracking.
        let notified_packet = {
            let dp = self.decoy_providers.read().await.get(&identity).cloned();
            if let Some(dp) = dp {
                let notified = dp.feed_output(body, trailer_buf.clone()).await;
                match notified {
                    None => return Ok(()),
                    Some(b) => b.expand_end(trailer_len),
                }
            } else {
                body.expand_end(trailer_len)
            }
        };
        ...
        // Fallthrough decoys: drop the plaintext trailer, skip encryption, treat the remaining body as opaque random bytes.  Non-fallthrough path is unchanged.
        let (encrypted_packet, packet_flags, data_len, trailer_overhead) = if fallthrough {
            let body_only = notified_packet.rebuffer_end(notified_packet.len() - trailer_len);
            let body_len = body_only.len();
            (body_only, PacketFlags::DECOY, body_len, 0_usize)
        } else {
            let (packet_data, packet_trailer) = notified_packet.split_buf_end(trailer_len);
            let flags = PacketFlags::from_bits_truncate(*packet_trailer.get(0));
            let data_len = packet_data.len();
            let encrypted_trailer = {
                let mut crypto = self.crypto_send.lock().await;
                crypto.obfuscate_trailer(packet_trailer, self.settings.pool()).await.map_err(FlowControllerError::TrailerEncryption)?
            };
            let encrypted = packet_data.expand_end(encrypted_trailer.len());
            (encrypted, flags, data_len, TRAILER_S2C_OVERHEAD)
        };
        ...
        record_server_send(addr, || {
            ...
            let trailer_len = if fallthrough {
                0
            } else {
                Trailer::<T>::len()
            };
            (kind, trailer_len, trailer_overhead, cap_header, data_len, cap_body)
        });
        Ok(())
    }
}
